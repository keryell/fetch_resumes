  ...
  for (auto i = 1; i < argc; ++i) {
    ...
    for (const auto & line : getlines_view { file } | views::tail) {
      ...
      auto url = line
        ...
        // \todo handle full CSV parsing with cases such as "containing.."
        | views::split(',')
        // Pick the element of the 9th column where the URL is
        | views::slice(8, 9)
        // Transform the subrange of characters generated by split
        // back to a string
        | views::transform(to<std::string>());
